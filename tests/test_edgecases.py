def test_update_nonexistent_returns_404(client):
    # no todos exist
    res = client.put(
        "/todos/999", json={"title": "no one"}
    )  # yei yei check krne kei liye likha hai jab todo exist hei na hau
    assert res.status_code == 405


def test_delete_nonexistent_is_idempotent(client):
    # deleting non-existing still return 200 and message)
...
def test_create_todo_without_title(client):
    ...
    assert res.status_code == 400, "bhai khali nhi chalega"
...
def test_mark_todo_done(client):
    ...
    data = res.get_json()
    assert data["is_done"] is True, " this is done"
...
def test_deleteto_request(client):
    ...
    res2 = client.post("/todos", json={"title": "today i to much tierd"})
    res3 = client.post("/todos", json={"title": "today i do my project"})
